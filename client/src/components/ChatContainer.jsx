...
const ChatContainer = () => {
  ...
  // ticker to update "now" every 30s so Edit labels expire in real time
  useEffect(() => {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (data?.success) {
        ...
        // show short animation/flash locally
  ...
  // load messages + scrolling + active chat notify
  useEffect(() => {
  ...
  // utility: returns true if we should show Edit label for this message
  const shouldShowEditLabel = (msg) => {
    // If backend provided editedAt, use it to decide
    if (msg.editedAt) {
      const editedAtMs = new Date(msg.editedAt).getTime();
      if (Number.isNaN(editedAtMs)) return !!msg.edited; // fallback
      return Date.now() - editedAtMs < TEN_MINUTES_MS;
    }
          ...
          return (
            ...
            >
              {msg.deleted ? (
                ...
                <div className="relative mb-8 overflow-visible">
                  ...
                  {confirmForMessage === msg._id && (
                    <div className="absolute -top-12 right-0 w-[220px] bg-gray-900 rounded-md p-2 shadow-lg z-50" onClick={(e) => e.stopPropagation()}>
                      <p className="text-sm text-gray-200 mb-2">Delete this message?</p>
                      {disabledBySeen && <p className="text-xs text-gray-400 mb-2">Message seen — delete & edit disabled.</p>}
                      <div className="flex justify-end gap-2">
                        <button onClick={() => setConfirmForMessage(null)} className="px-2 py-1 rounded bg-white/5 text-gray-200 hover:bg-white/10 text-sm">Cancel</button>
                        <button onClick={() => confirmDelete(msg._id)} className={`px-2 py-1 rounded text-sm ${disabledBySeen ? 'bg-gray-600 text-gray-300 cursor-not-allowed' : 'bg-red-600 text-white hover:bg-red-700'}`} disabled={disabledBySeen}>Delete</button>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="relative mb-8 max-w-[220px] overflow-visible">
                  <div className="relative p-2 rounded-lg break-words bg-violet-500/30 text-white max-w-full overflow-visible">
                    <p className={`md:text-sm font-light ${isMine ? 'text-right' : 'text-left'}`} style={{ wordWrap: 'break-word' }}>
                      {msg.text}
                    </p>
                  ...
                  {confirmForMessage === msg._id && (
                    <div className="absolute -top-12 right-0 w-[240px] bg-gray-900 rounded-md p-2 shadow-lg z-50" onClick={(e) => e.stopPropagation()}>
                      <p className="text-sm text-gray-200 mb-2">Choose action</p>
                      {disabledBySeen && <p className="text-xs text-gray-400 mb-2">Message seen — delete & edit disabled.</p>}
                      <div className="flex justify-end gap-2">
                        <button onClick={() => setConfirmForMessage(null)} className="px-2 py-1 rounded bg-white/5 text-gray-200 hover:bg-white/10 text-sm">Cancel</button>
                        <button onClick={() => startEdit(msg)} className={`px-2 py-1 rounded text-sm ${disabledBySeen || msg.image ? 'bg-gray-600 text-gray-300 cursor-not-allowed' : 'bg-yellow-600 text-white hover:bg-yellow-700'}`} disabled={disabledBySeen || msg.image} title={msg.image ? 'Cannot edit image messages' : undefined}>Edit</button>
                        <button onClick={() => confirmDelete(msg._id)} className={`px-2 py-1 rounded text-sm ${disabledBySeen ? 'bg-gray-600 text-gray-300 cursor-not-allowed' : 'bg-red-600 text-white hover:bg-red-700'}`} disabled={disabledBySeen}>Delete</button>
                      </div>
                    </div>
                  )}
              ...
              <div className="text-center text-xs">
                <img className="w-7 rounded-full" src={isMine ? authUser?.profilePic || assets.avatar_icon : selectedUser?.profilePic || assets.avatar_icon} alt="" />
        ...
        <div className="flex-1 flex items-center bg-gray-100/12 px-3 rounded-full">
          ...
          {editingMessageId ? (
            <div className="flex items-center gap-2 ml-2">
              <span className="px-2 py-1 rounded bg-yellow-600 text-black text-sm font-medium">Editing…</span>
              <button onClick={cancelEdit} className="px-2 py-1 rounded bg-white/5 text-gray-200 hover:bg-white/10 text-sm">Cancel</button>
            </div>
          ) : null}
        </div>
